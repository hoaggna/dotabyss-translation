...
Các lỗi được xử lý:
1. 旦那様 (dannasama) dịch sai thành "chồng", "chồng bạn", "thưa ngài", "ông xã", "Chủ nhân điện hạ"... -> sửa thành "Chủ nhân".
...
def fix_translation(ja: str, vi: str) -> tuple[str, list[str]]:
    ...
    if ja == "はい、旦那様がよいのです。" and ("chồng" in vi.lower() or "vẫn ổn" in vi.lower()):
    ...
    # 8. Danh xưng 旦那様 -> Chủ nhân
    if "旦那様" in ja:
        prev = vi
        # Bỏ 'Chủ nhân điện hạ'
        vi = re.sub(r"\bChủ nhân điện hạ\b", "Chủ nhân", vi, flags=re.I)

        # 'だ、旦那様' -> 'C-Chủ nhân' / 'Đ-Chủ nhân'
        vi = re.sub(r"\b[Dd]-(?:thưa ngài|chồng)\b", "C-Chủ nhân", vi)
        ...
        vi = re.sub(r"\bthanh thịt của chồng bạn\b", "thanh thịt của Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bthanh thịt của chồng em\b", "thanh thịt của Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bthanh thịt của chồng\b", "thanh thịt của Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bhạt giống của chồng em\b", "tinh hoa của Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bhạt giống của chồng\b", "tinh hoa của Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bcon Chủ nhân tràn ngập\b", "tinh hoa của Chủ nhân tràn ra", vi, flags=re.I)

        # Các cụm 'chồng bạn', 'chồng em', 'người chồng', 'ông xã'
        vi = re.sub(r"\bchồng bạn\b", "Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bchồng em\b", "Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bchồng tôi\b", "Chủ nhân", vi, flags=re.I)
        vi = re.sub(r"\bchồng mình\b", "Chủ nhân", vi, flags=re.I)
